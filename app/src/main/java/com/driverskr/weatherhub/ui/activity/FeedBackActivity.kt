...
import com.driverskr.weatherhub.databinding.ActivityFeedBackBinding
import com.driverskr.weatherhub.ui.activity.vm.FeedBackViewModel
import com.driverskr.weatherhub.ui.base.BaseActivity
import com.driverskr.weatherhub.utils.DeviceUtil
...
class FeedBackActivity: BaseActivity<ActivityFeedBackBinding>() {
    ...
    fun getEmei() {
        PermissionUtils(this).requestPermissions(arrayOf(android.Manifest.permission.READ_PHONE_STATE)) {
            if (it) {
                logE(TAG, DeviceUtil.getCPUSerial(context))
            } else {
                toast("没有权限")
            }
        }
