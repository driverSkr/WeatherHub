package com.driverskr.lib.specialeffects
...
import com.driverskr.lib.specialeffects.entity.Cloud
import java.util.*
...
class EffectCloudDrawable(val clouds: Array<Drawable>) : Drawable(), Animatable {
    ...
    private fun startAnim() {
        logD("EffectCloudDrawable","isActive: "+scope.isActive)
        scope.launch {
            while (isActive) {
    ...
    override fun stop() {
        scope.cancel()
        logD("EffectCloudDrawable","Effect2Drawable cancel ---------------------------> ")
    }
